...
6. `## Options` - always use an Options section, never an `## API` section. A "Pass an options object as the second argument:" line with a one-line code example (`await pkg(url, { theOption: value })`), then a table (`Option | Required | Default | Description`). If the package takes no options, omit the section entirely - document the arguments inline in Usage instead. Watch verb precision in descriptions ("include", not "fetch", when nothing extra is downloaded).
